...
export default function Navbar() {
  ...
  const isActive = (href: string) => {
    const active = activeHash.toLowerCase().replace('#/', '#');
    const target = href.toLowerCase().replace('#/', '#');
    if (target === '#' || target === '' || target === '#home') {
      return active === '#' || active === '' || active === '#home' || active === '#/';
    }
                  ...
                  <li key={link.name}>
                    <a
                      ...
                      className={`relative py-2 text-sm font-semibold transition-colors duration-200 ${
                        active ? 'text-brand-royal' : 'text-brand-navy/90 hover:text-brand-royal'
                      }`}
                    >
                      {link.name}
              ...
              {navLinks.map((link) => (
                <li key={link.name}>
                  <a
                    ...
                    className={`block text-base font-semibold py-2 border-b border-brand-gray-dark/50 ${
                      isActive(link.href) ? 'text-brand-royal' : 'text-brand-navy/90 hover:text-brand-royal'
                    }`}
                  >
                    {link.name}
                  </a>
